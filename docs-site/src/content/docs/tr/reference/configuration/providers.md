...
| `models?` | `string[]` | Tohum/geri dönüş listesi. `liveModels: false` iken bu liste, boşsa `defaultModel` kullanılır. |
...
Yalnızca yapılandırılmış modelleri göstermek için `liveModels: false` ayarlayın. `models` boşsa
veya atlanırsa yapılandırılmış `defaultModel` tek model olarak gösterilir; o da yoksa yönlendirilmiş model gösterilmez. Canlı keşif,

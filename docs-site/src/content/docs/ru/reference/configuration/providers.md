...
| `models?` | `string[]` | Seed/fallback-список. При `liveModels: false` используется он, а если он пуст — `defaultModel`. |
...
Задайте `liveModels: false`, чтобы показывать только настроенные модели. Если `models` пуст или отсутствует,
провайдер показывает настроенный `defaultModel` как единственную модель; если нет и его — не показывает маршрутизируемых моделей. Live-discovery отвергает ответы

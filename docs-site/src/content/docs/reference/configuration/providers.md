...
| `models?` | `string[]` | Seed/fallback model list. With `liveModels: false`, this list is used, or `defaultModel` when the list is empty. |
...
Set `liveModels: false` to expose only configured models. If `models` is empty or omitted, the provider
exposes `defaultModel` when configured; otherwise it exposes no routed models. Live discovery rejects more than 4 MiB or 2,000 raw model rows before caching;

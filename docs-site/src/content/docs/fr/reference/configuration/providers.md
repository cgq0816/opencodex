...
| `models?` | `string[]` | Liste initiale ou de repli. Avec `liveModels: false`, cette liste est utilisée, ou `defaultModel` si elle est vide. |
...
Réglez `liveModels: false` pour exposer uniquement les modèles configurés. Si `models` est vide ou omis, le fournisseur
expose `defaultModel` s'il est configuré ; sinon aucun modèle routé. La découverte dynamique rejette plus de 4 Mio ou 2 000 lignes de modèle brutes avant leur mise en cache ;

...
| `models?` | `string[]` | 种子/回退模型列表。配合 `liveModels: false` 时使用此列表；列表为空时使用 `defaultModel`。 |
...
将 `liveModels: false` 设为只暴露已配置模型。如果 `models` 为空或省略，已配置的 `defaultModel` 会作为唯一模型暴露；两者都没有时才不暴露路由模型。实时发现会在缓存前拒绝超过 4 MiB 或 2,000 条原始模型行；内置预设可能使用更低的限制，并过滤为可聊天的行。过大或格式错误的结果会走陈旧/配置回退。合法的、零可用结果的发现仍然具有权威性，不会被静默替换或截断。
